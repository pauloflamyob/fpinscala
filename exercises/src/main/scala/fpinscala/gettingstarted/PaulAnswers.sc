...
object PolymorphicFunctions {
  ...
  def curry[A, B, C](uncurriedFunction: (A, B) => C): A => (B => C) =
    a => b => uncurriedFunction(a, b)
  def curryExample(uberEatsDeliveryTotal: (Int, Int) => Int): Int => (Int => Int) =
    deliveryFee => price => uberEatsDeliveryTotal(deliveryFee, price)

  // NB: The `Function2` trait has a `curried` method already

  // Exercise 4: Implement `uncurry`
  def uncurry[A, B, C](curriedFunction: A => B => C): (A, B) => C =
    (a, b) => curriedFunction(a)(b)
  def uncurryExample(uberEatsDeliveryTotal: Int => (Int => Int)): (Int, Int) => Int =
    (deliveryFee, price) => uberEatsDeliveryTotal(deliveryFee)(price)
